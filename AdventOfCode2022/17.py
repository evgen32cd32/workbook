...
with open('/Users/evgeny/python/workbook/data/advent2022/advent_17.txt','r') as f:
    cmds = f.readline()[:-1]
            ...
            if min([area[coord[1]][x] + area[coord[1] + 1][x] for x in range(len(area[0]))]) > 0:
                ...
                for y in range(len(area)-1):
                    ...
                    for x in range(len(area[0])):
                        score = score + area[y][x] * 2**x
                state = (ifig, icmd, score)
                if state in saveStates:
                    deltaI = i - saveStates[state][0]
                    deltaH = hidden - saveStates[state][1]
                    rocksLeft = rocksCnt - i
                    cyclesLeft = rocksLeft // deltaI
                    hidden = hidden + cyclesLeft * deltaH
                    rocksCnt = rocksLeft % deltaI
                    i = 0
                    saveStates.clear()
                else:
                    saveStates[state] = (i, hidden)
